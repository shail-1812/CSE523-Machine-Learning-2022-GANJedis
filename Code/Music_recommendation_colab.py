...
#     y = data.groupby('year')[category].mean()
#     ax1.plot(data['year'].unique(), y, label=category, linewidth=1)
...
# ax1.legend(prop={'size': 10}, borderpad=0.55, labelspacing=0.5, loc=9)
# ax2.legend(prop={'size': 10}, borderpad=0.55, labelspacing=0.5, loc='upper right')
# plt.show()
...
# X, y = data[feature_names], data['intensity']
# X = scaler.fit_transform(X)
...
# tsne_pipeline = Pipeline([('scaler', StandardScaler()), ('PCA', PCA(n_components=2, verbose=1))])
# genre_embedding = tsne_pipeline.fit_transform(X)
# projection = pd.DataFrame(columns=['x', 'y'], data=genre_embedding)
# projection['genres'] = genre_data['genres']
# projection['cluster'] = genre_data['cluster']
...
# pca_pipeline = Pipeline([('scaler', StandardScaler()), ('TSNE', TSNE(n_components=2, verbose = 1))])
# song_embedding = pca_pipeline.fit_transform(X)
# projection = pd.DataFrame(columns=['x', 'y'], data=song_embedding)
# projection['title'] = data['name']
# projection['cluster'] = data['cluster_label']
